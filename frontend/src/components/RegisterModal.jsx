...
const YEARS = Array.from({ length: 110 }, (_, i) => (new Date().getFullYear() - i).toString());
...
function RegisterModal({ isOpen, onClose, onOpenLogin, onError, registerFunction }) {
    ...
    // Verificar si el email ya está registrado (llamada al backend)
    const checkEmailExists = async (email) => {
        try {
            ...
            if (!data.available) {
                setEmailError('Este correo ya está registrado. ¿Ya tienes cuenta? Inicia sesión.')
                return false
            }
            return true
        } catch {
    ...
    const validateDateOfBirth = (date) => {
        ...
        if (parts.length < 3) return 'La fecha de nacimiento es requerida'

        const [yearStr, monthStr, dayStr] = parts
        if (!yearStr || !monthStr || !dayStr) return 'La fecha de nacimiento es requerida'

        const year = parseInt(yearStr, 10)
        const month = parseInt(monthStr, 10)
        const day = parseInt(dayStr, 10)

        // Crear fecha y validar que sea una fecha real (evita roll-over de JS)
        const birthDate = new Date(year, month - 1, day)

        if (birthDate.getFullYear() !== year || birthDate.getMonth() !== month - 1 || birthDate.getDate() !== day) {
            return 'La fecha seleccionada no es válida para ese mes'
        }
    ...
    const handleChange = (e) => {
        ...
        // Manejar cambios en partes de la fecha
        if (name === 'dob_day' || name === 'dob_month' || name === 'dob_year') {
            const currentDob = formData.date_of_birth || '--'
            let [y, m, d] = currentDob.split('-')

            if (name === 'dob_day') d = value ? value.padStart(2, '0') : ''
            if (name === 'dob_month') m = value ? value.padStart(2, '0') : ''
            if (name === 'dob_year') y = value || ''

            const newDate = `${y}-${m}-${d}`
            setFormData(prev => ({ ...prev, date_of_birth: newDate }))

            // Validar siempre que cambie algo, pero el JSX decidirá si mostrarlo
            if (y && m && d) {
                setDateError(validateDateOfBirth(newDate))
            } else {
                setDateError('') // Limpiar error si la fecha está incompleta
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        const emailAvailable = await checkEmailExists(formData.email)
        if (!emailAvailable) {
            const msg = emailError || 'Este correo ya está registrado.'
            setError(msg)
        ...
        try {
            ...
            // Manejar error de email duplicado que pueda venir del backend
            if (data?.errors?.email) {
                const emailMsg = Array.isArray(data.errors.email) ? data.errors.email[0] : data.errors.email
                const friendlyMsg = 'Este correo ya está registrado. ¿Ya tienes cuenta? Inicia sesión.'
                setEmailError(friendlyMsg)
                setError(friendlyMsg)
                if (onError) onError(friendlyMsg)
            } else {
                const errorMsg = data?.message || err.message || 'Error al registrarse'
                setError(errorMsg)
                if (onError) onError(errorMsg)
            }
        } finally {
                ...
                <form onSubmit={handleSubmit} className="auth-form">
                    ...
                    <div className="form-group">
                        ...
                        <input
                            ...
                            onBlur={async (e) => {
                                ...
                                if (val) {
                                    const fmtError = validateEmail(val)
                                    if (fmtError) {
                                        setEmailError(fmtError)
                                    } else {
                                        await checkEmailExists(val)
                                    }
                                }
                            }}
                            required
                            disabled={loading || emailChecking}
                            className={emailError ? 'input-error-border' : emailChecking ? 'input-checking' : ''}
                        />
                        {emailChecking && <small className="hint-text">⏳ Verificando disponibilidad...</small>}
                        {!emailChecking && emailError && <small className="error-text">✗ {emailError}</small>}
                        {!emailChecking && !emailError && formData.email && <small className="success-text">✓ Correo disponible</small>}
                        {!emailChecking && !formData.email && <small className="hint-text">Usa un correo válido para tu cuenta</small>}
                    </div>
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1.5fr', gap: '10px' }}>
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[2] && !dateError ? '#44cc44' : 'rgba(212, 167, 106, 0.15)'}`,
                                }}
                            >
                            ...
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[1] && !dateError ? '#44cc44' : 'rgba(212, 167, 106, 0.15)'}`,
                                }}
                            >
                            ...
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[0] && !dateError ? '#44cc44' : 'rgba(212, 167, 106, 0.15)'}`,
                                }}
                            >
                        ...
                        {formData.date_of_birth && formData.date_of_birth.split('-').every(p => p !== '') ? (
                            dateError ? (
                    ...
                    <div className="form-group">
                        ...
                        <select
                            ...
                            disabled={loading}
                            style={{
                                border: `2px solid ${genderError && formData.gender === '' ? '#ff4444' : formData.gender ? '#44cc44' : 'rgba(212, 167, 106, 0.15)'}`,
                            }}
                        >
                    ...
                    <div className="form-group">
                        ...
                        <select
                            ...
                            disabled={loading}
                            style={{
                                border: `2px solid ${countryError && formData.country === '' ? '#ff4444' : formData.country ? '#44cc44' : 'rgba(212, 167, 106, 0.15)'}`,
                            }}
                        >
                    ...
                    <button
                        ...
                        disabled={loading || emailChecking || passwordStrength.score < 50 || !!nameError || !!emailError || !!passwordError || !!passwordConfirmationError || !!dateError || !!genderError || !!countryError || !formData.name || !formData.email || !formData.password || !formData.password_confirmation || !formData.date_of_birth || !formData.gender || !formData.country}
                    >
                        {loading ? 'Creando cuenta...' : emailChecking ? 'Verificando...' : 'Crear Cuenta'}
                    </button>
                </form>
